...
POSITIONED_LABELS = ["O"] + ["%s-%s"%(bilu, label) for label in LABELS for bilu in "I"]
...
class UnifiedAnnotator(annotations.BaseAnnotator):
    ...
    def annotate(self, doc):
        ...
        while i < len(predicted):
            if predicted[i]!= "O":
                if predicted[i].startswith("U-") or predicted[i].startswith("II-") or predicted[i].startswith("L-"):
                    conf = round(confidences[i],3)
                    doc.user_data["annotations"][self.source_name][(i, i+1)] = ((predicted[i][2:], conf),)
                    i += 1 
                elif predicted[i].startswith("I-"):
                    start = i
                    label = predicted[i][2:]
                    i += 1
                    while i < (len(doc)) and predicted[i] != "O" and predicted[i].startswith("I-"):
                        i += 1
#                     if i < len(doc) and predicted[i].startswith("L-"):
                    conf = round(confidences[start:i].max().mean(),3)
                    doc.user_data["annotations"][self.source_name][(start, i)] = ((label, conf),)
                    # i += 1
            else:
                i += 1 
        return doc 
        ...
        for i, source in enumerate(SOURCE_NAMES):
            ...
            for (start,end), vals in doc.user_data["annotations"][source].items():
                for label, conf in vals:
                    ...
                    sequence[start:end,i, LABEL_INDICES["I-%s"%label]] = conf  
#                     if end-start==1:
        ...
        for source_index, source in enumerate(SOURCE_NAMES):
            ...
            for pos_index, pos_label in enumerate(POSITIONED_LABELS[1:]):
                if pos_label[2:] in SOURCE_PRIORS[source]:
                    obs_counts[source_index,pos_index+1] += 1
...
class SnorkelModel(UnifiedAnnotator):
    ...
    def train(self, docbin_file):
        ...
        self.label_model = snorkel.labeling.model.LabelModel(len(LABELS) + 1)
